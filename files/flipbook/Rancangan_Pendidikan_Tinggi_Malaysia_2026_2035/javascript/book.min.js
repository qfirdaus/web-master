...
try{console&&console.error&&console.error.apply(console,arguments)}catch(b){}this.logForPad(String.format.apply(this,arguments),"red")}},print:function(){var b=String.format.apply(this,arguments),c=(new Date).format("hh:mm:ss.S");this.debug("[%s] %s",c,b)}}}),VERSION_NAME="6.4.9",BUILD_DATE="2023100801",BookType={normal_book:0,single_book:1,slide_book:3,singleSlide_book:6,catalog_book:7,singleCatalog_book:8},normalBookStatu={book_statu_ready:0,book_statu_auto_flip:1,book_statu_mouse_flip:2,book_statu_mag_flip:3},
...
19+1,f.length)));0<=f.indexOf("maxheighttosmallmode")&&(c=parseInt(f.substring(f.indexOf("maxheighttosmallmode")+20+1,f.length)))}return{w:b,h:c}},init:function(){if(!bookConfig.hideMiniFullscreen&&!isIphone()){if(isPhone()||isPad())if(window.self!==window.top){var b=window.innerHeight,c=window.screen.height,d=0;try{d=window.top.innerHeight}catch(e){d=0}if(70<=100*(d?b/d:b/c)||window.location.href.includes("bookcase"))return}else return;this.caption||(this.caption=$("<div class='clickToRead'></div>"),
this.caption.html(getLanguage("lblFullscreen","Click to view in fullscreen.")),tmpContainer.append(this.caption),this.initEvent())}},initEvent:function(){this.caption.bind(_event._end,function(){this.isMini()&&readAsFullscreen()}.bind(this))},hide:function(){this.caption&&this.caption.hide()},show:function(){this.caption&&this.caption.show()},destroy:function(){this.caption&&(this.caption.remove(),this.caption=void 0)}}});
