...
class SppagebuilderAddonCarouselpro extends SppagebuilderAddons
{
	...
	public function render()
	{
		...
		{
			shuffle($settings->sp_carouselpro_item);
		}
		...
		if (isset($settings->sp_carouselpro_item) && count((array) $settings->sp_carouselpro_item)) {
			foreach ($settings->sp_carouselpro_item as $key => $value) {
				...
				if ($video) {
					...
					$output  .= ($item_image_src) ? '<img class="sppb-img-reponsive" src="' . $item_image_src . '" alt="' . $alt_text . '">' : '';
				}
	...
	public function css()
	{
		...
		$transformCss = $cssHelper->generateTransformStyle('.sppb-carousel-item-inner', $settings, 'transform');
		$css .= $transformCss;
	...
	public static function getTemplate()
	{
		...
		$output .= '
			...
			<# if(data.randomize_carousel){
				data.sp_carouselpro_item = _.shuffle(data.sp_carouselpro_item);
			} #>
		...
		$output .= $lodash->generateTransformCss('.sppb-carousel-item-inner', 'data.transform');
			...
			<div class="sppb-carousel-inner">
				...
				#>
					...
					<# if(_.isObject(data.carousel_height)){
						if(carouselBg.src){ #>
							<img class="sppb-carousel-pro-bg-image" src=\'{{ carouselBg.src }}\' alt="{{ alt_text }}">
						<# }
					} #>
								...
								<# } #>
									<div class="sppb-row">
										...
										<div class="sppb-col-sm-{{imageColumn}} sppb-col-xs-12">
											...
											#>
														<img class="sppb-img-reponsive" src=\'{{ pagebuilder_base + carouselItemImg.src }}\' alt="{{ alt_text }}">
													<# } else if(carouselItemImg.src){ #>
														<img class="sppb-img-reponsive" src=\'{{ carouselItemImg.src }}\' alt="{{ alt_text }}">
													<# }
												}
											} #>
											</div>
