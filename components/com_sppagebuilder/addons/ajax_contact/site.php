...
class SppagebuilderAddonAjax_contact extends SppagebuilderAddons
{
    ...
    public function render()
    {
        ...
        $_SESSION['isContactEnabledCaptcha_' . $this->addon->id] = (isset($settings->formcaptcha) && $settings->formcaptcha) ? true : false;
        ...
        $output .= '<input type="text" name="sppb-hpt_' . uniqid() . '" autocomplete="off" tabindex="-1"  class="sppb-hidden-xs sppb-hidden-sm sppb-hidden-md sppb-hidden-lg sppb-hidden-xl sppb-hidden-xxl sppb-hidden-print"/>';
        ...
        if ($formcaptcha && $captcha_type == 'default') {
        ...
        } elseif ($formcaptcha && ($captcha_type == 'recaptcha_invisible') || $captcha_type == 'igcaptcha') {
            ...
            if ($formcaptcha) {
                ...
                if ($captcha_type === 'powcaptcha') {
                    ...
                    $recaptcha = Factory::getApplication()->triggerEvent('onDisplay', [null, 'custom_captcha_' . $this->addon->id, 'sppb-dynamic-recaptcha']);
    ...
    public static function getAjax()
    {
        ...
        foreach ($inputs as $input) {
            ...
            if (mb_strpos($input['name'], 'sppb-hpt_') === 0 && ! empty($input['value'])) {
                return json_encode(['status' => false, 'content' => '<span class="sppb-text-danger">Restricted Access</span>']);
            }
        ...
        $showcaptcha = isset($_SESSION['isContactEnabledCaptcha_' . $addonId]) ? $_SESSION['isContactEnabledCaptcha_' . $addonId] : true;
        ...
        if ($mail->sendMail($senderMail, $senderName, $recipient, $mailSubject, $mailBody, $isHtmlMode, $cc, $bcc, $attachment, $email, $name)) {
            if (!empty($send_copy_to_applicant)) {
                ...
                $copyMailSubject = Text::_('COM_SPPAGEBUILDER_ADDON_AJAX_CONTACT_SEND_COPY_MAIL_SUBJECT') . $subject;
                $copyMailBody    = '<div><p>' . Text::_('COM_SPPAGEBUILDER_ADDON_AJAX_CONTACT_SEND_COPY_MAIL_HEADER') . ':</p>' . $mailBody . '</div>';

                if ($copyMail->sendMail($senderMail, $senderName, $email, $copyMailSubject, $copyMailBody, $isHtmlMode)) {
                    $output['status']  = true;
    ...
    public function css()
    {
        ...
        if ($use_custom_button) {
            $css .= $css_path->render(['addon_id' => $addon_id, 'options' => $this->addon->settings, 'id' => 'btn-' . $this->addon->id]);
        }
        ...
        $transformCss          = $cssHelper->generateTransformStyle('.sppb-ajax-contact-content', $settings, 'transform');
    ...
    public static function getTemplate()
    {
        ...
        $output .= $lodash->generateTransformCss('.sppb-ajax-contact-content', 'data.transform');
        $output .= '
        </style>
		<div class="sppb-addon sppb-addon-ajax-contact {{ data.class }}">
			...
			<div class="sppb-ajax-contact-content">
				<form class="sppb-ajaxt-contact-form">
                    ...
                    <# if(isCaptchaEnabled && data.captcha_type == "recaptcha"){ #>
                        <div class="sppb-row">
					...
					<# } #>
                    <# if(isCaptchaEnabled && data.captcha_type == "recaptcha_invisible"){ #>
                        <div class="sppb-row">
                            <div class="sppb-form-group sppb-col-sm-12">
					...
					<# } #>
					 <# if(isCaptchaEnabled && (data.captcha_type !== "recaptcha_invisible") && (data.captcha_type !== "recaptcha") && (data.captcha_type !== "default")){ #>
                        <div class="sppb-row">
                            <div class="sppb-form-group sppb-col-sm-12">
