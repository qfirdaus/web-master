...
class SppagebuilderAddonModal extends SppagebuilderAddons
{
	...
	public function render()
	{
		...
		$selector_original_image_width = (isset($selector_image->width) && $selector_image->width) ? $selector_image->width : '';
		$selector_original_image_height = (isset($selector_image->height) && $selector_image->height) ? $selector_image->height : '';
		$selector_image_width = (isset($settings->selector_image_width) && $settings->selector_image_width) ? $settings->selector_image_width : $selector_original_image_width;
		$selector_image_height = (isset($settings->selector_image_height) && $settings->selector_image_height) ? $settings->selector_image_height : $selector_original_image_height;
		...
		if ($modal_selector === 'image') {
			if ($selector_image_src) {
				...
				<img ' . ($placeholder ? 'class="sppb-element-lazy modal-selector-image"' : '') . ' src="' . ($placeholder ? $placeholder : $image_link) . '" alt="' . $selector_image_alt_text . '" ' . ($placeholder ? 'data-large="' . $image_link . '"' : '') . ' ' . ($selector_image_width ? 'width="' . $selector_image_width . '"' : '') . ' ' . ($selector_image_height ? 'height="' . $selector_image_height . '"' : '') . ' loading="lazy" />';
	...
	public static function getTemplate()
	{
		...
		let modalSelectorType = data.modal_selector || ""
		let modalSelectorImageHeight = data.selector_image_height || ""
		...
		$output .= $lodash->generateTransformCss('.sppb-modal-selector', 'data.transform');
	    ...
	    <# } #>
			...
			var selector_original_image_width = data.selector_image_width || "";
			var selector_original_image_height = data.selector_image_height || "";
			var selector_image_width = data.selector_image_width || selector_original_image_width;
			var selector_image_height = data.selector_image_height || selector_original_image_height;
			...
			#>
				<a class="sppb-modal-selector sppb-magnific-popup" {{{ attribs }}} href=\'{{ modalUrl }}\' id="{{ modalUniqueId }}-selector">
					<# if(selector_image.src && selector_image.src.indexOf("https://") == -1 && selector_image.src.indexOf("http://") == -1){ #>
						<img class="modal-selector-image" src=\'{{ pagebuilder_base + selector_image.src }}\' alt=\'{{selector_image_alt_text}}\' width=\'{{ selector_image_width }}\' height=\'{{ selector_image_height }}\' >
					<# } else { #>
						<img class="modal-selector-image" src=\'{{ selector_image.src }}\' alt=\'{{selector_image_alt_text}}\' width=\'{{ selector_image_width }}\' height=\'{{ selector_image_height }}\' >
					<# } #>
