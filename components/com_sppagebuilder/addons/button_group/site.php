...
class SppagebuilderAddonButton_group extends SppagebuilderAddons
{
	...
	public function js() {
		...
		if (isset($settings->sp_button_group_item) && count((array) $settings->sp_button_group_item)) {
			...
			$jquery = '';
			foreach ($settings->sp_button_group_item as $key => $value) {
				$scriptsArray = $model->generatePopup('#btn-' . $this->addon->id . $key, $value);
				if (!is_array($scriptsArray)) {
					$jquery .= ' ' . $scriptsArray;
				} else {
					array_push($popupScripts, $scriptsArray);
					if (isset($popupScripts[$key])) {
						$doc->addScriptDeclaration('
							window.htmlAddContent = !window?.htmlAddContent ? `' . $popupScripts[$key][0] . '` : window.htmlAddContent += `' . $popupScripts[$key][0] . '`;
						');
					}
				}
			}

			foreach ($popupScripts as $popupScript) {
				if (!empty($popupScript)) {
					$doc->addScriptDeclaration($popupScript[1]);
					$doc->addStyleDeclaration($popupScript[2]);
					$doc->addScriptDeclaration($popupScript[3]);
					$doc->addScriptDeclaration($popupScript[4]);
				}
			}
