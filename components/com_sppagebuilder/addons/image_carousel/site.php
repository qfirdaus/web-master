...
class SppagebuilderAddonImage_carousel extends SppagebuilderAddons
{
    ...
    public function render()
    {
        ...
        if (isset($settings->sp_image_carousel_item) && is_array($settings->sp_image_carousel_item))
        {
            ...
            {
                shuffle($settings->sp_image_carousel_item);
            }
            ...
            foreach ($settings->sp_image_carousel_item as $item_key => $carousel_item)
            {
                ...
                $item_title = isset($carousel_item->item_title) ? $carousel_item->item_title : '';
                $alt_text = isset($image_carousel_img->alt) ? $image_carousel_img->alt : $item_title;
                ...
                $output .= '<img src="' . $image_carousel_img_src . '" alt="' . $alt_text . '">';
    ...
    public function css()
    {
		...
		$transformCss = $cssHelper->generateTransformStyle('.sppb-carousel-extended', $settings, 'transform');
    ...
    public static function getTemplate()
    {
		...
		$output .= $lodash->generateTransformCss('.sppb-carousel-extended', 'data.transform');
        ...
        let carousel_margin_xl = 15;
        let carousel_margin_lg = 15;
        let carousel_margin_md = 15;
        let carousel_margin_sm = 15;
        let carousel_margin_xs = 15;
            ...
            {{{data.image_carousel_layout == "layout1" && data.carousel_fade ?  `data-fade="${data.carousel_fade}"` : ""}}}>
                ...
                if(data.randomize_carousel){
                    ...
                    _.each(data.sp_image_carousel_item, function(carousel_item){
                        ...
                        let alt_text = carousel_item?.image_carousel_img?.alt ?? carousel_item?.item_title;
                #>
                        <div class="sppb-carousel-extended-item">
                            ...
                            #>
                                <img src=\'{{ pagebuilder_base + carouselImg.src }}\' alt="{{ alt_text }}">
                            <# } else if(carouselImg.src){ #>
                                <img src=\'{{ carouselImg.src }}\' alt="{{ alt_text }}">
                            <# }
