...
class SppagebuilderAddonCarousel extends SppagebuilderAddons
{
	...
	public function render()
	{
		...
		{
			shuffle($settings->sp_carousel_item);
		}
		...
		if (isset($settings->sp_carousel_item) && count((array) $settings->sp_carousel_item))
		{
			foreach ($settings->sp_carousel_item as $key => $value)
			{
				...
				if ((isset($value->title) && $value->title) || (isset($value->content) && $value->content))
				{
					...
					$output  .= (isset($value->content) && $value->content) ? '<div id="addon-content-'. $this->addon->id ."-". $key  .'" class="sppb-carousel-content">' . $value->content . '</div>' : '';
	...
	public function css()
	{
		...
		foreach ($this->addon->settings->sp_carousel_item as $key => $value)
		{
			...
			$itemTitleMarginStyle = $cssHelper->generateStyle($title_id_selector, $value, ['title_margin' => 'margin', 'title_padding' => 'padding'], ['title_margin' => false, 'title_padding' => false], ['title_margin' => 'spacing', 'title_padding' => 'spacing']);
			$css .= $itemTitleMarginStyle;

			// Content Margin
			$content_id_selector = '#addon-content-'. $this->addon->id .'-'. $key;
			$itemContentMarginStyle = $cssHelper->generateStyle($content_id_selector, $value, ['content_margin' => 'margin', 'content_padding' => 'padding'], ['content_margin' => false, 'content_padding' => 'padding'], ['content_margin' => 'spacing', 'content_padding' => 'spacing']);
		...
		$transformCss = $cssHelper->generateTransformStyle('.sppb-carousel', $settings, 'transform');
		$css .= $transformCss;
	...
	public static function getTemplate()
	{
		...
		$output .= '<# } #>';
		$output .= $lodash->generateTransformCss('.sppb-carousel', 'data.transform');
