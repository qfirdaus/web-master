...
class SppagebuilderAddonPerson extends SppagebuilderAddons
{
	...
	public function render()
	{
		...
		$alt_text = (isset($image->alt) && $image->alt) ? $image->alt : $name;
		...
		if ($image_src) {
			...
			$output  .= '<img class="sppb-img-responsive' . ($placeholder ? ' sppb-element-lazy' : '') . '" style="display: inline-block;" src="' . ($placeholder ? $placeholder : $image_src) . '" alt="' . $alt_text . '" ' . ($placeholder ? 'data-large="' . $image_src . '"' : '') . ' ' . ($image_width ? 'width="' . $image_width . '"' : '') . ' ' . ($image_height ? 'height="' . $image_height . '"' : '') . ' loading="lazy">';
	...
	public function css()
	{
		...
		$transformCss = $cssHelper->generateTransformStyle('.sppb-addon-person', $settings, 'transform');
	...
	public static function getTemplate()
	{
		...
		$output .= $lodash->generateTransformCss('.sppb-addon-person', 'data.transform');
		...
		$output .= '
			...
			if (!_.isEmpty(data.twitter)) {
				social_icon_list += `<li><a target="_blank" href="${data.twitter}"><i class="'. $twitterIcon .'"></i></a></li>`;
			}
				...
				#>
					<div class="sppb-person-image <# if(data.person_style_preset=="layout4"){ #> person-layout-4 <# } #>">
						<# if(personImg.src.indexOf("https://") == -1 && personImg.src.indexOf("http://") == -1){ #>
							<img class="sppb-img-responsive" style="display: inline-block;" src=\'{{ pagebuilder_base + personImg.src }}\' alt="{{ alt_text ?? data.name }}">
						<# } else { #>
							<img class="sppb-img-responsive" style="display: inline-block;" src=\'{{ personImg.src }}\' alt="{{ alt_text ?? data.name }}">
						<# } #>
