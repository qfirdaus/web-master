...
class SppagebuilderAddonNavigation extends SppagebuilderAddons
{
	...
	public function render()
	{
		...
		if ($sticky_menu)
		{
			$sticky_row_attr = ' data-sticky-it-nav="true"';
		}
		...
		if (count((array) $links))
		{
			foreach ($links as $key => $link)
			{
				...
				$scroll_to_attr = ($scroll_to && isset($linkUrl) && strpos($linkUrl, '#') !== false) ? ' data-scroll-to="true" ' : '';
	...
	public function css()
	{
		...
		$transformCss = $cssHelper->generateTransformStyle('.sppb-link-list-wrap', $settings, 'transform');
	...
	public static function getTemplate()
	{
		...
		$output .= $lodash->generateTransformCss('.sppb-link-list-wrap', 'data.transform');

		$output .= '
        </style>
        <div class="sppb-link-list-wrap {{ nav_type }} {{ nav_align }} {{ responsive_menu_cls }} {{data.class}}" {{{ sticky_row_attr }}}>
            <# if(responsive_menu){ #>
                <div class="sppb-responsive-bars" aria-label="{{responsive_bar_aria_label}}"><span class="sppb-responsive-bar"></span><span class="sppb-responsive-bar"></span><span class="sppb-responsive-bar"></span></div>
            <# } #>
