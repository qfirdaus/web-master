...
class SppagebuilderAddonIcon extends SppagebuilderAddons
{
	...
	public function render()
	{
		...
		if ($name) {
			...
			if (!empty($link)) {
				$output .= '<a ' . $target . ' href="' . $link . '"' . ' ' . $ariaLabel . '>';
			}
	...
	public static function getTemplate()
	{
		...
		<# 
		let hover_effect_class = (!_.isEmpty(data.hover_effect) && data.hover_effect) ? ` sppb-icon-hover-effect-${data.hover_effect}` : "";

		if (data.name) { #>
			<div class="sppb-icon {{ data.class }} {{ hover_effect_class }}">
				<# 
				...
				if (isMenu || isPage || isUrl || isOldUrl) {
					...
					#>
					<a href=\'{{ newUrl }}\' target=\'{{ target }}\' rel=\'{{ rel }}\' aria-label=\'{{ ariaLabel }}\'>
				<# }
				...
				<span class="sppb-icon-inner">
					...
					<span class="sppb-form-label-visually-hidden">{{ icon_title }}</span>
				</span>
				<# if (isMenu || isPage || isUrl || isOldUrl) { #>
					</a>
				<# } #>
