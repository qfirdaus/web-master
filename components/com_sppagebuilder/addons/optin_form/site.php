...
class SppagebuilderAddonOptin_form extends SppagebuilderAddons
{
	...
	public function render()
	{
		...
		elseif ($platform == 'acymailing')
		{
			...
			if (version_compare($acym_version, '11.0', '>='))
			{
				$acymailing_helper = rtrim(JPATH_ADMINISTRATOR, '/') . '/components/com_acym/Core/init.php';
			}
			elseif (version_compare($acym_version, '6.0', '>='))
			{
		...
		if ($recaptcha && ($captcha_type == 'recaptcha' || $captcha_type == 'gcaptcha'))
		{
			...
			$output .= '</div>'; //.sppb-form-group
		} elseif ($recaptcha && ($captcha_type == 'recaptcha_invisible' || $captcha_type == 'igcaptcha')) {
	...
	public static function getAjax()
	{
		...
		if (isset($showcaptcha) && $showcaptcha)
		{
			if($captcha_type !== 'default') {
				...
				} else if ($captcha_type == 'recaptcha' || $captcha_type == 'gcaptcha') {
					PluginHelper::importPlugin('captcha', 'recaptcha');
		...
		{ // if AcyMailing
			// if acymailing isn't installed
			if (version_compare($acymversion, '11.0', '>='))
			{
				$acymailing_helper = rtrim(JPATH_ADMINISTRATOR, '/') . '/components/com_acym/Core/init.php';
			}
			elseif (version_compare($acymversion, '6.0', '>='))
			{
				$acymailing_helper = rtrim(JPATH_ADMINISTRATOR, '/') . '/components/com_acym/helpers/helper.php';
			}
			else
			...
			if (version_compare($acymversion, '6.0', '>='))
			{ // if version is more than or equal 6
				...
				{
					$acy_list_class = new \AcyMailing\Classes\ListClass();
