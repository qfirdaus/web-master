...
class SppagebuilderAddonArticles_scroller extends SppagebuilderAddons
{
	...
	public function render()
	{
		...
		$ticker_heading = (isset($settings->ticker_heading)) ? $settings->ticker_heading : Text::_('COM_SPPAGEBUILDER_ADDON_ARTICLE_SCROLLER_TICKER_HEADING');
		...
		if (count((array) $items)) {
			...
			if ($addon_style === 'scroller') {
				...
				foreach ($items as $key => $item) {
					...
					$output .= '<div class="sppb-articles-scroller-date-left-content" role="article">';
				...
				foreach ($items as $key => $item) {
					$output .= '<div role="article" class="sppb-articles-ticker-text ' . $show_shape_class . '">';
				...
				$aria_label = Text::_('COM_SPPAGEBUILDER_NAVIGATION_SCROLLER_ARIA_LABEL');

				$output .= '</div>'; //.sppb-articles-ticker-content
				$output .= '<div class="sppb-articles-ticker-controller" role="navigation" aria-label="' . $aria_label . '">';
	...
	public function css()
	{
		...
		$css .= $cssHelper->generateStyle('.sppb-articles-scroller-meta-date-left span, .sppb-articles-ticker-heading', $settings, ['left_text_color' => 'color'], false);
		...
		$transformCss = $cssHelper->generateTransformStyle('.sppb-articles-ticker-wrap', $settings, 'transform');
		$css .= $transformCss;
		...
		$carouselTitleTypographyStyle = $cssHelper->typography('.sppb-articles-carousel-link', $settings, 'carousel_title_typography', $typographyFallbacks);
		$carouselDateTypographyStyle = $cssHelper->typography('.sppb-articles-carousel-meta-date', $settings, 'carousel_date_typography', $typographyFallbacks);
		$carouselTextTypographyStyle = $cssHelper->typography('.sppb-articles-carousel-introtext', $settings, 'carousel_text_typography', $typographyFallbacks);
		$carouselCategoryTypographyStyle = $cssHelper->typography('.sppb-articles-carousel-meta-category a', $settings, 'carousel_category_typography', $typographyFallbacks);
		$tickerHeadingTypography = $cssHelper->typography('.sppb-articles-ticker-heading, .sppb-articles-scroller-day', $settings, 'ticker_heading_typography', $typographyFallbacks);
		$contentTypography = $cssHelper->typography('.sppb-articles-ticker-text a, .sppb-addon-articles-scroller-title', $settings, 'content_typography', $typographyFallbacks);
	...
	public function js()
	{
		...
		$aria_label_previous = Text::_('COM_SPPAGEBUILDER_NAVIGATION_SCROLLER_PREVIOUS');
		$aria_label_next = Text::_('COM_SPPAGEBUILDER_NAVIGATION_SCROLLER_NEXT');
					...
					jQuery("' . $addon_id . ' .sppb-articles-ticker-content").bxSlider({
						...
						autoStart:true,
						 onSliderLoad: function() {
										jQuery("' . $addon_id . ' .sppb-articles-ticker-left-control a.bx-prev").attr({
											"aria-label": "'. $aria_label_previous .'",
											"role": "button",
											"href": "#"
										});
										jQuery("' . $addon_id . ' .sppb-articles-ticker-right-control a.bx-next").attr({
											"aria-label": "'. $aria_label_next .'",
											"role": "button",
											"href": "#"
										});
                					  }
