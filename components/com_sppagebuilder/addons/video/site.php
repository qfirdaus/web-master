...
class SppagebuilderAddonVideo extends SppagebuilderAddons
{
	...
	public function render()
	{
		...
		$video_aria_label = (isset($settings->video_aria_label) && $settings->video_aria_label) ? $settings->video_aria_label : '';
		$video_aria_described_by = (isset($settings->video_aria_described_by) && $settings->video_aria_described_by) ? $settings->video_aria_described_by : '';
	...
	public static function getTemplate()
	{
		...
		$output = '
			...
			<#
				...
				let video_title		  = "";
				const video_aria_label = data?.video_aria_label || "";
				let video_aria_described_by = data?.video_aria_described_by || "";
				let show_rel_video 	  = (typeof data.show_rel_video !== "undefined" && data.show_rel_video) ? "&rel=1" : "&rel=0";
				let embedSrc 		  = ""
		...
		$output .= $lodash->generateTransformCss('.sppb-addon-video', 'data.transform');
				 ...
				 <# } else {
					if(mp4_video.src || ogv_video.src){
						...
						<div class="sppb-addon-video-local-video-wrap">
							<video class="sppb-addon-video-local-source" 							<# if(!_.isEmpty(video_aria_label)) { #>	
								aria-label=\'{{ video_aria_label }}\'
							<# } #>
							<# if(!_.isEmpty(video_aria_described_by)) { #>
							aria-describedby=\'{{ video_aria_described_by }}\'
							<# } #>
							{{(data.video_loop != 0 ? " loop" : "")}}{{(data.autoplay_video != 0 ? " autoplay" : "")}}{{(data.show_control != 0 ? " controls" : "")}}{{(data.video_mute != 0 ? " muted" : "")}}
