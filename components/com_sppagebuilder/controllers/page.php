...
class SppagebuilderControllerPage extends FormController
{
	...
	public function __construct($config = array())
	{
		...
		if(!$canEdit && $canEditOwn) {
			...
			$pageCreatorId = $model->getPageCreatorId($pageId);
		}

		$canEditPage = $canEdit || ($canEditOwn && $user->id === $pageCreatorId);
	...
	public function save($key = null, $urlVar = null)
	{
		...
		switch ($task)
		{
			case 'apply':
				...
				{
					$model->addArticleFullText($oldPage->view_id, $oldPage->content);
				}
				...
				{
					FilesystemFile::delete($css_file_path);
				}
	...
	public function getOrCreateStorePage()
	{
		...
		// Derive the layout for the canvas: prefer the saved `content` column, fall back
		// to `text` (fresh pages), matching how the editor loads content on first render.
		$db = Factory::getDbo();
		...
		if (!empty($row))
		{
			$rawContent = !empty($row->content) ? $row->content : (!empty($row->text) ? $row->text : '[]');
		}
		...
		echo json_encode(['status' => true, 'id' => $id, 'content' => $content, 'redirect' => $redirect]);
		die();
	}
	...
	public function getPageForm()
	{
		...
		foreach ($groups as $name => $group)
		{
			...
			foreach ($fields as $field)
			{
				...
				$fieldArray['placeholder'] = isset($fieldArray['placeholder']) ? Text::_($fieldArray['placeholder']) : '';

				$value = $form->getValue($fieldName);
				$fieldArray['value'] = $value;

				if($fieldName === 'language') {
					$fieldArray['options'] = $model->getLanguages();
	...
	public function savePopupSettings()
	{
		...
		$response = $model->savePopupSettingsData($data, $id);
	...
	public function importLayout()
	{
		...
		$id = $input->get('id', 0, 'INT');
		$type = $input->get('type', 'template', 'STRING');
	...
	public function savePage()
	{
		...
		try
		{
			...
			if (!$model->savePage((array) $pageData))
			{
				$response = [
					'status' => false,
	...
	public function loadPopupsList(){
		$result = [];
		...
		$query->select('id AS value, title AS label')
			->from($db->quoteName('#__sppagebuilder'))
			->where($db->quoteName('extension_view') . '=' . $db->quote(Page::PAGE_TYPE_POPUP))
			->where($db->quoteName('published') . ' = 1');
