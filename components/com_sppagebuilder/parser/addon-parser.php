...
class AddonParser
{
	...
	public static function viewAddons($content, $fluid = 0, $pageName = 'none', $level = 1, $newModule = true, $storeData = [], $isReset = false)
	{
		...
		if ((empty(self::$contents) || $isReset) && $pageName != 'module')
		{
			...
			foreach ($content as $row)
			{
				...
				foreach ($row->columns as $column)
				{
					...
					foreach ($column->addons as $key => $addon)
					{
						...
						{
							$output .= self::getDivHTMLView($addon, $column->addons, $layouts, $pageName, $storeData, 0, [
								'rowId' => $row->id,
								'colId' => $column->id
							]);
						}
	...
	private const COLLECTION_ADDONS = ['easystore_common_productlist'];
	...
	public static function getCollectionHTMLView($addon, $layouts, $pageName, $storeData = [])
	{
		...
		if (!empty($addon->name))
		{
			if (in_array($addon->name, self::COLLECTION_ADDONS))
			{
	 ...
	 * @since 	4.0.0
	 */
	public static function getDivHTMLView($addon, &$addonList, $layouts, $pageName, $storeData = [], $storeListIndex = 0, $ids = [])
	{
		$divElement = '';
		...
		$rowId = !empty($ids['rowId']) ? $ids['rowId'] : null;
		$colId = !empty($ids['colId']) ? $ids['colId'] : null;
		...
		if (!empty($addon->name))
		{
			if ($addon->name === 'div')
			{
				...
				$lazyLoadClass = $lazyload ? ' sppb-element-lazy ' : '';

				$responsive_hidden_class = "";
				$responsive_hidden_class .= (isset($addon->settings->hidden_xl) && filter_var($addon->settings->hidden_xl, FILTER_VALIDATE_BOOLEAN)) ? 'sppb-hidden-xl ' : '';
				$responsive_hidden_class .= (isset($addon->settings->hidden_lg) && filter_var($addon->settings->hidden_lg, FILTER_VALIDATE_BOOLEAN)) ? 'sppb-hidden-lg ' : '';
				$responsive_hidden_class .= (isset($addon->settings->hidden_md) && filter_var($addon->settings->hidden_md, FILTER_VALIDATE_BOOLEAN)) ? 'sppb-hidden-md ' : '';
				$responsive_hidden_class .= (isset($addon->settings->hidden_sm) && filter_var($addon->settings->hidden_sm, FILTER_VALIDATE_BOOLEAN)) ? 'sppb-hidden-sm ' : '';
				$responsive_hidden_class .= (isset($addon->settings->hidden_xs) && filter_var($addon->settings->hidden_xs, FILTER_VALIDATE_BOOLEAN)) ? 'sppb-hidden-xs ' : '';
				...
				$global_select_position = (isset($addon->settings->global_seclect_position) && $addon->settings->global_seclect_position) ? $addon->settings->global_seclect_position : '';

				$divElement .= self::generateDivCSS($addon, $pageName, $layouts);
				
				$divElement .= '<div id="sppb-addon-wrapper-' . $addon->id . '" class="sppb-addon-wrapper ' . $responsive_hidden_class . '">';
				$divElement .= '<div id="sppb-addon-' . $addon->id . '" class="sppb-div-addon' . $lazyLoadClass . ' ' . $addon->settings->class . $animation['class'] . ($global_custom_position && $global_select_position != '' ? ' sppb-positioned-addon ' : '') . '" ' . $animation['attributes'] . ($global_section_z_index ? 'data-zindex="' . $global_section_z_index . '"' : '') . ' ' . ($global_addon_z_index ? 'data-col-zindex="' . $global_addon_z_index . '"' : '') . ' ' . ($global_custom_position && $rowId ? 'data-rowid="' . $rowId . '"' : '') . ' ' . ($global_custom_position && $colId ? 'data-colid="' . $colId . '"' : '') . '>';
				$divElement .= $addon->settings->global_use_overlay == 1 ? '<div class="sppb-addon-overlayer"></div>' : '';
			}
			else
			{
		...
		if (!empty($addon->children))
		{
			foreach ($addon->children as $child)
			{
				...
				/** Addon Visibility */
				if (isset($childAddon->visibility) && !$childAddon->visibility)
				{
				...
				$divElement .= self::getDivHTMLView($childAddon, $addonList, $layouts, $pageName, $storeData, $storeListIndex, $ids);
				$addonList[$index]->visited = true;
			}
		}
	...
	/**
	 * Get the HTML view for the dynamic content DIV addon
	 *
	 ...
	 * @param callable $renderCollectionAddon The callback function to render the child collection addon
	 * @param array $data The data to be rendered
	 * @param int $index The index of the item
	 * @param bool $generateCss Whether to generate CSS
	 * @return string The rendered content
	 * 
	 * @since 5.5.0
	 */
	public static function getDivHTMLViewForDynamicContent($addon, $layouts, $pageName, $renderCollectionAddon, $index = 0)
	{
		$output = '';
		$animation = self::generateAnimation($addon);
		$custom_class = "";
		...
		$css = static::generateDivCSS($addon, $pageName, $layouts);

		$output .= $css;
		$output .= '<div id="sppb-addon-wrapper-' . $addon->id . '" class="sppb-addon-wrapper">';
		$output .= '<div id="sppb-addon-' . $addon->id . '" class="sppb-div-addon ' . $addon->settings->class . $custom_class . $animation['class'] . '" ' . $animation['attributes'] . '>';

		if (empty($addon->child_nodes)) {
			$output .= '</div></div>';
			return $output;
		}

		$childNodes = $addon->child_nodes;

		foreach ($childNodes as $childNode) {
			if (empty($childNode->visibility)) {
				continue;
			}

			if (!static::checkAddonACL($childNode)) {
				continue;
			}

			// Convey the dynamic item to the child node
			$childNode->settings->dynamic_item = $addon->settings->dynamic_item;

			if ($childNode->name === 'div') {
				$output .= self::getDivHTMLViewForDynamicContent($childNode, $layouts, $pageName, $renderCollectionAddon, $index);
			} elseif ($childNode->name === 'dynamic_content_collection') {
				$output .= $renderCollectionAddon($childNode);
			} else {
				$output .= self::getAddonHtmlView($childNode, $layouts, $pageName, false, [], $index, false);
			}
		}
	...
	public static function getDivHTMLViewForCollection($addon, &$addonList, $layouts, $pageName, $storeData = [], $storeListIndex = 0, $isGenerateCss = false)
	{
		$divElement = '';
			...
			if ($addon->name === 'div')
			{
				...
				if (!empty($addon->children))
				{
					foreach ($addon->children as $child)
					{
						...
						$divElement .= self::getDivHTMLViewForCollection($childAddon, $addonList, $layouts, $pageName, $storeData, $storeListIndex, $isGenerateCss);
					}
				}
			}
			else
			{
				$divElement .= self::getAddonHtmlView($addon, $layouts, $pageName, true, $storeData, $storeListIndex, $isGenerateCss);
			}
		}
	...
	public static function extractDeviceSettings(&$addon)
	{
		$store = new \stdClass;
	...
	public static function getAddonHtmlView($addon, $layouts, $pageName = 'none', $isChild = false, $storeData = [], $storeListIndex = 0, $isGenerateCss = false)
	{
		/**
		...
		if (file_exists($addon_path . '/site.php'))
		{
			...
			self::extractDeviceSettings($addon);
			...
			{
				$items = self::processRepeatableItems($hasRepeatableItems, $addon->name, $layouts, $pageName, $storeData);
			}
			...
			if (class_exists($class_name))
			{
				...
				if (method_exists($class_name, 'css'))
				{
					...
					if ($isGenerateCss)
					{
						$cssString = self::convertCssArrayToString(self::minifyCss(self::$css_content));
						$doc->addStyleDeclaration($cssString);
					}
				}
			}
			else
