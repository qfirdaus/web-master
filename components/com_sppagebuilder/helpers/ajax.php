...
if ($action === 'upload-page')
{
	...
	{
		echo json_encode(array('status' => false, 'data' => 'Session token is invalid. Please try again.'));
		die;
	}
	...
	if (!$authorised)
	{
		echo json_encode(array('status' => false, 'data' => 'You are not authorised to perform this action.'));
		die;
	}
...
function importLayoutWithMedia($zipFilePath, $zipFileName)
{
	...
	if ($zip->open($zipFilePath) === true)
	{
		...
		if (!extractZipSafely($zip, $extractedPath))
		{
			$zip->close();
			Folder::delete($extractedPath);

			echo json_encode(array('status' => false, 'data' => 'Invalid zip file contents.'));
			die;
		}
		$zip->close();
...
function extractZipSafely(ZipArchive $zip, string $destinationPath)
{
	for ($i = 0; $i < $zip->numFiles; $i++)
	{
		$entryName = $zip->getNameIndex($i);

		if ($entryName === false || !isSafeZipEntryPath($entryName))
		{
			return false;
		}
	}
...
function isSafeZipEntryPath(string $entryName)
{
