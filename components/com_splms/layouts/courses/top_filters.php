...
if (is_array($cate_filters)) {
    ...
    $cate_filters = ($cate_filters) ? explode(',', $cate_filters) : $cate_filters;
    $cate_filters = ArrayHelper::toInteger($cate_filters);
}

$category_id  = $input->get('category', NULL, 'RAW');

// Filter by levels
$level_filters  = $input->get('level', NULL, 'STRING');
$level_filters  = ($level_filters) ? explode(',', $level_filters) : $level_filters;
...
<div class="splms-courses-filters-top">
    ...
    <?php if ($category_id && ($menu_params->get('show_subcategory') == 1)) { ?>
        ...
        <div>
            <form action="<?php echo Route::_('index.php?option=com_splms&view=courses', false); ?>" id="splms-courses-filters-form" class="splms-courses-filters">
                <ul class="splms-filter-option">
                    <?php if ($categories) { ?>
                        <li class='selected btn btn-sm btn-primary splms-filter-option-li' data-target="all">
                            <?php echo Text::_('COM_SPLMS_SHUFFLE_SHOW_ALL'); ?>
                        </li>
                        <?php foreach ($categories as $key => $category) { ?>
                            <li class="btn btn-sm btn-primary splms-filter-option-li" data-target="<?php echo $category->alias; ?>">
                        ...
                        <?php foreach ($categories as $key => $category) { ?>
                            <li>
                                <label class="custom-checkbox">
                                    <input <?php echo ($menu_params->get('show_subcategory') == 1) ? 'type="radio"' : 'type="checkbox"'; ?> value="<?php echo $category->id; ?>" name="category[]" <?php echo (in_array($category->id, $cate_filters)) ? 'checked="checked"' : ''; ?>>
                ...
                <div class="splms-course-filter-levels">
                    ...
                    <ul id="splms-filter-levels" class="splms-filter-items">
                        <?php foreach ($levels as $key => $level) { ?>
                            <li>
                                <label class="custom-checkbox">
                                    <input type="checkbox" name="level[]" value="<?php echo strtolower($level); ?>" <?php echo ($level_filters && in_array(strtolower($level), $level_filters)) ? 'checked="checked"' : ''; ?>>
