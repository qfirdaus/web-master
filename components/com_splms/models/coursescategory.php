...
class SplmsModelCoursescategory extends ItemModel {

	protected $_context = 'com_splms.coursescategory';

	protected function populateState() {
		$app = Factory::getApplication('site');
		$itemId = $app->input->getInt('id');
		$this->setState('coursescategory.id', $itemId);
	...
	public function getItem( $itemId = null ) {
		...
		$itemId = (!empty($itemId))? $itemId : (int)$this->getState('coursescategory.id');
		...
		if (!isset($this->_item[$itemId])) {
			try {
				...
				$query->select('a.*');
				$query->from('#__splms_coursescategories as a');
