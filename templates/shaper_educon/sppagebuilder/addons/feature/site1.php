	...
	class SppagebuilderAddonFeature extends SppagebuilderAddons
	{
		...
		public function render()
		{
			...
			//Button
			$btn_class = '';
			$btn_class .= (isset($this->addon->settings->type) && $this->addon->settings->type) ? ' sppb-btn-' . $this->addon->settings->type : '';
			$btn_class .= (isset($this->addon->settings->size) && $this->addon->settings->size) ? ' sppb-btn-' . $this->addon->settings->size : '';
			$btn_class .= (isset($this->addon->settings->block) && $this->addon->settings->block) ? ' ' . $this->addon->settings->block : '';
			$btn_class .= (isset($this->addon->settings->shape) && $this->addon->settings->shape) ? ' sppb-btn-' . $this->addon->settings->shape : ' sppb-btn-rounded';
			$btn_class .= (isset($this->addon->settings->appearance) && $this->addon->settings->appearance) ? ' sppb-btn-' . $this->addon->settings->appearance : '';
			...
			if ($button_text != '') {
				if ($icon_position == 'left') {
					$button_text = ($icon) ? '<i class="fa ' . $icon . '"></i> ' . $button_text : $button_text;
				} else {
					$button_text = ($icon) ? $button_text . ' <i class="fa ' . $icon . '"></i>' : $button_text;
				}
			}
			...
			if ($icon_image_position == 'before') {
				$output .= ($media) ? $media : '';
				$output .= '<div class="sppb-addon-content-wrap">';
				$output .= ($title) ? $feature_title : '';
				$output .= $feature_text;
				//Button
				if ($button_text != '') {
					$output .= '<a' . $attribs . ' class="sppb-btn ' . $btn_class . '">' . $button_text . '</a>';
				}
				$output .= '</div>'; //End .sppb-addon-content-wrap
		...
		public static function getTemplate()
		{
			...
			<#
				...
				var btn_class = "";
					btn_class += (data.type) ? "sppb-btn-" + data.type + " " : "";
					btn_class += (data.size) ? "sppb-btn-" + data.size + " " : "";
					btn_class += (data.block) ? " " + data.block + " " : "";
					btn_class += (data.shape) ? "sppb-btn-" + data.shape + " " : "sppb-btn-rounded ";
					btn_class += (data.appearance) ? "sppb-btn-" + data.appearance + " " : "";

				var attribs = "";
					attribs += (data.target) ? \' target="\' + data.target + \'"\' : "";
					attribs += (data.url) ? \' href="\' + data.url + \'"\' : "";
					attribs += \' id="btn-\' + data.id + \'"\';
		
				var button_text = "";
				if (!_.isEmpty(data.button_text)) {
					if (data.icon_position == \'left\') {
						button_text += (data.icon) ? \'<i class="fa \'+ data.icon+ \'"></i> \'+ data.button_text : data.button_text;
					} else if (data.icon_position == \'right\') {
						button_text += (data.icon) ? data.button_text +\' <i class="fa \' + data.icon + \'"></i>\' : data.button_text;
					}
				}
				...
				<div class="sppb-addon-content">
					<# if (icon_image_position == "before") { #>
						<# if(media){ #>
							{{{ media }}}
						<# } #>
						<div class="sppb-addon-content-wrap">
						<# if(data.title){ #>
							{{{ feature_title }}}
						<# } #>
						{{{ feature_text }}}
						<# if (!_.isEmpty(button_text)) { #>
							<a {{attribs}} class="sppb-btn {{btn_class}}">{{{button_text}}}</a>
						<# } #>
						</div>
					<# } else if (icon_image_position == "after") { #>
						<div class="sppb-addon-content-wrap">
