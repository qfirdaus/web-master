...
class SppagebuilderAddonArticles extends SppagebuilderAddons
{
	...
	public function render()
	{
		...
		if ($isk2installed == 0 && $resource == 'k2') {
		...
		if (count($items)) {
			...
			foreach ($items as $key => $item) {
				...
				if ($show_author || $show_category || $show_date) {
					...
					if ($show_date) {
						$output .= '<span class="sppb-meta-date" itemprop="dateCreated"><i class="fa fa-clock-o"></i> ' . Jhtml::_('date', $item->created, 'M d, Y') . '</span>';
					}
					...
					if ($show_category) {
						...
						$output .= '<span class="sppb-meta-category"><i class="fa fa-th"></i> <a href="' . $item->catUrl . '" itemprop="genre">' . $item->category . '</a></span>';
					}

					if ($show_author) {
						$output .= '<span class="sppb-meta-author" itemprop="name"><i class="fa fa-user"></i> ' . $item->username . '</span>';
					}
				...
				if ($show_readmore) {
					$output .= '<a class="sppb-readmore sppb-btn sppb-btn-primary" href="' . $item->link . '" itemprop="url">' . $readmore_text . ' <i class="fa fa-angle-right"></i></a>';
				}
				$output .= '<span class="count-hits pull-right"><i class="fa fa-eye"></i>' . $item->hits . '</span>';
