...
class SppagebuilderAddonAnimated_number extends SppagebuilderAddons
{
	...
	public function render()
	{
		...
		$syntax = (isset($this->addon->settings->syntax) && $this->addon->settings->syntax) ? $this->addon->settings->syntax : '';
		...
		$counter_title_alignment = (isset($this->addon->settings->counter_title_alignment) && $this->addon->settings->counter_title_alignment) ? $this->addon->settings->counter_title_alignment : 'before';
		...
		if ($counter_title && $counter_title_alignment == 'before') {
			$output .= '<div class="sppb-animated-number-title">' . $counter_title . '</div>';
		}

		$output .= '<div class="sppb-animated-number-wrap clearfix">';
		$output .= '<div class="sppb-animated-number pull-left" data-digit="' . $number . '" data-duration="' . $duration . '">0</div>';

		if ($syntax != 0) {
			$output .= '<div class="sppb-animated-number-syntax pull-left">' . $syntax . '</div>';
		}

		$output .= '</div>'; //.sppb-animated-number-wrap

		if ($counter_title && $counter_title_alignment == 'after') {
			$output .= '<div class="sppb-animated-number-title">' . $counter_title . '</div>';
		}
	...
	public static function getTemplate()
	{
		...
		<#
			...
			var counter_title_alignment = data.counter_title_alignment || "before";
		#>
		<style type="text/css">
		...
		<div class="sppb-addon sppb-addon-animated-number {{ data.alignment }} {{ data.class }}">
			<div class="sppb-addon-content">
				...
				<# if (counter_title_alignment == \'before\') { #>
					<div class="sppb-animated-number-title">{{data.counter_title}}</div>
				<# } #>
				
				<div class="sppb-animated-number-wrap clearfix">
				<div class="sppb-animated-number pull-left" data-digit="{{ data.number }}" data-duration="{{ data.duration }}">0</div>

				<# if (!_.isEmpty(data.syntax)) { #>
					<div class="sppb-animated-number-syntax pull-left">{{data.syntax}}</div>
				<# } #>
				</div>
				
				<# if (counter_title_alignment == \'after\') { #>
					<div class="sppb-animated-number-title">{{data.counter_title}}</div>
				<# } #>
			</div>
		</div>';
