...
$moduleHtml = preg_replace(
	...
	'\\1 <a class="btn btn-link jmodedit" data-bs-toggle="tooltip" data-bs-placement="top" title="' . Text::_('JLIB_HTML_EDIT_MODULE') . ' , ' . htmlspecialchars($mod->title, ENT_COMPAT, 'UTF-8') . ' , ' . sprintf(Text::_('JLIB_HTML_EDIT_MODULE_IN_POSITION'), htmlspecialchars($position, ENT_COMPAT, 'UTF-8')) . '" href="' . $editUrl . '" target="' . $target . '" aria-describedby="tip-' . (int) $mod->id . '">
	<span class="icon-edit" aria-hidden="true"></span><span class="d-none">' . Text::_('JGLOBAL_EDIT') . '</span></a>',
	$moduleHtml,
	1,
	$count
);
