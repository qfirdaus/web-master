...
class modDJImageSliderHelper
{
		...
		switch($slide->params->get('link_type', '')) {
			...
			case 'article':
				if ($artid = $slide->params->get('id',$slide->params->get('link_article',0))) {
					...
					BaseModel::addIncludePath(JPATH_BASE.DS.'components'.DS.'com_content'.DS.'models');
	...
	static function getSlideTarget($link) {
		...
		if($link && preg_match("/^http/",$link) && !preg_match("/^".str_replace(array('/','.','-'), array('\/','\.','\-'),Uri::base())."/",$link)) {
			$target = '_blank';
		} else {
			$target = '_self';
		}
