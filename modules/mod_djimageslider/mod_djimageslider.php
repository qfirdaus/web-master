...
if($params->get('slider_source')==1) {
	...
	if(!ComponentHelper::isEnabled('com_djimageslider', true)){
		$app->enqueueMessage(Text::_('MOD_DJIMAGESLIDER_NO_COMPONENT'),'notice');
		return;
	}
	$slides = modDJImageSliderHelper::getImagesFromDJImageSlider($params);
	if($slides==null) {
		$app->enqueueMessage(Text::_('MOD_DJIMAGESLIDER_NO_CATEGORY_OR_ITEMS'),'notice');
		return;
	}
} else {
	$slides = modDJImageSliderHelper::getImagesFromFolder($params);
	if($slides==null) {
		$app->enqueueMessage(Text::_('MOD_DJIMAGESLIDER_NO_CATALOG_OR_FILES'),'notice');
		return;
	}
}
...
if ($jquery) {
	...
	if (version_compare(JVERSION, '4', '>=')) {
		$document->addScript(Uri::root(true).'/media/djextensions/jquery-easing-1.4.1/jquery.easing.min.js', array('mime'=>'text/javascript', 'defer'=>$canDefer));
	} else {
		$document->addScript(Uri::root(true).'/media/djextensions/jquery-easing/jquery.easing.min.js', array('mime'=>'text/javascript', 'defer'=>$canDefer));
	}
	$document->addScript(Uri::root(true).'/modules/mod_djimageslider/assets/js/slider.js?v='.$ver, array('mime'=>'text/javascript', 'defer'=>$canDefer));
} else {
	HTMLHelper::_('behavior.framework', true);
	$document->addScript(Uri::root(true).'/modules/mod_djimageslider/assets/js/moo.slider.js?v='.$ver, array('mime'=>'text/javascript', 'defer'=>$canDefer));
}

if($params->get('link_image',1) > 1) {
	if($jquery) {
		$document->addScript(Uri::root(true).'/media/djextensions/magnific/magnific.js', array('mime'=>'text/javascript', 'defer'=>$canDefer));
		$document->addStyleSheet(Uri::root(true).'/media/djextensions/magnific/magnific.css');
		$document->addScript(Uri::root(true).'/modules/mod_djimageslider/assets/js/magnific-init.js', array('mime'=>'text/javascript', 'defer'=>$canDefer));
	} else {
		$document->addScript(Uri::root(true).'/modules/mod_djimageslider/assets/slimbox/js/slimbox.js', array('mime'=>'text/javascript', 'defer'=>$canDefer));
		$document->addStyleSheet(Uri::root(true).'/modules/mod_djimageslider/assets/slimbox/css/slimbox.css');
	}
}
...
require ModuleHelper::getLayoutPath('mod_djimageslider', $params->get('layout','default'));
