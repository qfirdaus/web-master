...
class ControllerAjaxBrowse extends AdminAjaxController {
    ...
    public function actionUpload() {
        ...
        if (($params['restrict_uploads'] && strpos($params['upload_mime'], $mediaMime) === false) || !in_array($mediaMime, $allowedMimes)) {
            Notification::error(n2_('You are not allowed to upload this filetype!') . '<br>' . $mediaMime);
            $this->response->error();
        }
