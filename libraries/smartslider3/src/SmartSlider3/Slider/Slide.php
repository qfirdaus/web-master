...
class Slide extends AbstractRenderableOwner {
    ...
    public function finalize() {
        ...
        if ($this->sliderObject->exposeSlideData['lightboxImage']) {
            ...
            if (!empty($lightboxImage['url'])) {
                $this->attributes['data-lightbox-image'] = $lightboxImage['url'];
                $this->attributes['data-lightbox-alt']   = $lightboxImage['alt'];
            }
        }
    ...
    public function optimizeImageWebP($src) {
        if ($this->sliderObject->params->get('layer-image-webp', 0)) {
            ...
            if (isset($imagePath[0])) {
                ...
                if ($extension != 'svg') {
                    ...
                    return $this->sliderObject->features->optimize->optimizeImageWebP($src, array(
                        'optimize'     => true,
    ...
    public function renderImage($item, $src, $attributes = array(), $pictureAttributes = array()) {
        ...
        if (isset($imagePath[0])) {
            ...
            if ($extension != 'svg' && $item->data->get('image-optimize', 1)) {
                ...
                $retinaSupport = !!$this->sliderObject->params->get('layer-image-width-retina', 0);
    ...
    public function renderThumbnailImage($width, $height, $attributes = array()) {
        ...
        if (isset($imagePath[0])) {
            ...
            // strtok() strips any query string so remote AVIF URLs are detected correctly.
            $extension = strtolower(pathinfo(strtok($imagePath, '?'), PATHINFO_EXTENSION));
            $isAvif    = $extension === 'avif';

            /**
             * For AVIF we only convert the thumbnail to WebP when it is actually resized
             * ("Resize Thumbnail" on) and GD can decode AVIF (imagecreatefromavif). At full size a
             * WebP would be larger than the source AVIF, so we keep the original AVIF; if the AVIF
             * decoder is missing we also keep the original. JPEG/PNG still convert as before.
             */
            if ($extension && $this->sliderObject->params->get('optimize-webp', 0) && function_exists('imagewebp') && (!$isAvif || ($optimizeThumbnail && function_exists('imagecreatefromavif')))) {
                ...
                $quality = intval($this->sliderObject->params->get('optimize-thumbnail-quality', 70));
                ...
                if ($optimizeThumbnail) {
                    $options['mode']   = 'resize';
                    $options['width']  = intval($this->sliderObject->params->get('optimizeThumbnailWidth', 100));
                    $options['height'] = intval($this->sliderObject->params->get('optimizeThumbnailHeight', 60));
                } else {
                    $options['mode']  = 'scale';
                    $options['scale'] = 1;
                }

                $imageWebpUrl = ImageEdit::scaleImageWebp('slider/cache', $src, $options);

                if ($imageWebpUrl) {
                    $imageWebpUrl = ResourceTranslator::toUrl($imageWebpUrl);

                    /**
                     * Only emit the WebP source when a real conversion happened. If scaleImageWebp
                     * returned the original image unchanged (e.g. AVIF or a format GD cannot convert),
                     * a <source type="image/webp"> would point at a non-WebP file, so we skip it.
                     */
                    if ($imageWebpUrl !== ResourceTranslator::toUrl($src)) {
