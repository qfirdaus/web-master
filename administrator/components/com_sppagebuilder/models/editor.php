...
class SppagebuilderModelEditor extends AdminModel
{
    ...
    /**
     * Get the full readable route-style page type, including the articles / tags
     * special cases, matching the page list. e.g. "/articles" or "/articles/:slug".
     *
    ...
    private static function getPageTypeLabel($extensionView, $viewId = null)
    {
        ...
        if (!empty($collectionAlias)) {
            if ($extensionView === Page::PAGE_TYPE_DYNAMIC_CONTENT_INDEX) {
                ...
                $label = sprintf(Text::_('COM_SPPAGEBUILDER_EDITOR_PAGE_LIST_PAGE_TYPE_DYNAMIC_CONTENT_DETAIL'), $collectionAlias);
            }
        }
    ...
    /**
     * Reset the hit counter of a page by ID.
     *
     * @param int $id The page ID whose hits should be reset.
     *
     * @return bool True on success, false on failure.
     * @since 6.6.1
     */
    public function resetPageHits($id)
    {
        if (empty($id)) {
            return false;
        }
