...
trait Media
{
	...
	private function uploadMedia()
	{
		...
		$multipleResponse = $this->getInput('multiple', false, 'BOOL');
		$multiResponseData = [];
						...
						if (self::in_array($file_ext, $accepted_file_formats))
						{
							...
							if (File::upload($path, $dest, false, true))
							{
								...
								if ($multipleResponse)
								{
									...
									unset($updatedReport['status'], $updatedReport['message']);
									$multiResponseData[] = $updatedReport;
								}
							}
							else
							{
	...
	private function pathExistsInDB($path)
	{
		$db = Factory::getDbo();
		$query = $db->getQuery(true);
		$query->select('id')
