...
class SplmsModelCoursescategory extends AdminModel {
	/**
	...
	*/
	...
	public function getTable($type = 'Coursescategory', $prefix = 'SplmsTable', $config = array()) {
	...
	*/
	public function getForm($data = array(), $loadData = true) {
		// Get the form.
		$form = $this->loadForm('com_splms.coursescategory', 'coursescategory', array( 'control' => 'jform', 'load_data' => $loadData ) );
	...
	*/
	protected function loadFormData() {
		...
		$data = Factory::getApplication()->getUserState( 'com_splms.edit.coursescategory.data', array() );
	...
	public function save($data) {
		...
		if (in_array($input->get('task'), array('apply', 'save')) && (!isset($data['id']) || (int) $data['id'] == 0)) {
			if ($data['alias'] == null) {
				...
				$table = Table::getInstance('Coursescategory', 'SplmsTable');
	...
	*/
	protected function canDelete($record) {
		if (!empty($record->id)) {
			...
			return Factory::getUser()->authorise('core.delete', 'com_splms.coursescategory.' . (int) $record->id);
		}
