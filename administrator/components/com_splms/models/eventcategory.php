...
class SplmsModelEventcategory extends AdminModel {
	/**
	...
	*/
	...
	public function getTable($type = 'Eventcategory', $prefix = 'SplmsTable', $config = array()) {
	...
	*/
	public function getForm($data = array(), $loadData = true) {
		// Get the form.
		$form = $this->loadForm('com_splms.eventcategory', 'eventcategory', array( 'control' => 'jform', 'load_data' => $loadData ) );
	...
	*/
	...
	protected function loadFormData() {
		...
		$data = Factory::getApplication()->getUserState( 'com_splms.edit.eventcategory.data', array() );
		if (empty($data)) {
			$data = $this->getItem();
		}
	...
	public function save($data) {
		...
		if (in_array($input->get('task'), array('apply', 'save')) && (!isset($data['id']) || (int) $data['id'] == 0)) {
			if ($data['alias'] == null) {
				...
				$table = Table::getInstance('Eventcategory', 'SplmsTable');
	...
	*/
	protected function canDelete($record) {
		if (!empty($record->id)) {
			...
			return Factory::getUser()->authorise('core.delete', 'com_splms.eventcategory.' . (int) $record->id);
		}
