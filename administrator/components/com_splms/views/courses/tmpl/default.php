...
if ($saveOrder && !empty($this->items))
{
	if(SplmsHelper::getJoomlaVersion() < 4)
	{
		$saveOrderingUrl = 'index.php?option=com_splms&task=courses.saveOrderAjax&tmpl=component';
		HTMLHelper::_('sortablelist.sortable', 'courseList', 'adminForm', strtolower($listDirn), $saveOrderingUrl, false, true);
	}
	else
	{
		$saveOrderingUrl = 'index.php?option=com_splms&task=courses.saveOrderAjax&tmpl=component&' . Session::getFormToken() . '=1';
...
<form action="<?php echo Route::_('index.php?option=com_splms&view=courses'); ?>" method="post" id="adminForm" name="adminForm">
			...
			<table class="table table-striped" id="courseList">
				...
				<tr>
					...
					<th width="10%" class="nowrap">
						<?php echo HTMLHelper::_('searchtools.sort',  'COM_SPLMS_TITLE_PRICE', 'a.price', $listDirn, $listOrder); ?>
					</th>
				...
				<?php endif; ?>
					<?php if (!empty($this->items)) : ?>
						<?php foreach ($this->items as $i => $item) :
							...
							$canEdit    = $user->authorise('core.edit', 'com_splms.course.' . $item->id) || ($user->authorise('core.edit.own',   'com_splms.course.' . $item->id) && $item->created_by == $userId);
							$canCheckin = $user->authorise('core.manage', 'com_checkin') || $item->checked_out == $userId || $item->checked_out == 0;
							$canChange  = $user->authorise('core.edit.state', 'com_splms.course.' . $item->id) && $canCheckin;
							$link = Route::_('index.php?option=com_splms&task=course.edit&id=' . $item->id);
						?>
							<tr class="row<?php echo $i % 2; ?>" <?php echo SplmsHelper::getJoomlaVersion() < 4 ? 'sortable-group-id="1"' : 'data-draggable-group="1"';?>>
								...
								<td class="center">
									<?php echo HTMLHelper::_('jgrid.published', $item->published, $i, 'courses.', $canChange);?>
								</td>
								<td>
									...
									<?php if ($canEdit) : ?>
										<a href="<?php echo Route::_('index.php?option=com_splms&task=course.edit&id='.$item->id);?>">
								...
								<td class="hidden-phone">
									<?php if ($item->price == 0) {
										...
										$item->sale_price = ($item->sale_price > 0) ? $item->sale_price : '';
										echo SplmsHelper::getPrice($item->price, $item->sale_price);
									} ?>
								</td>
