...
jQuery(function($) {
    //Google Map
    $('.splms-gmap-address').after($('<div class="splms-gmap-canvas"></div>'));
    $('.splms-gmap-canvas').locationpicker({
        location: {latitude: $('.splms-gmap-address').data('latitude'), longitude: $('.splms-gmap-address').data('longitude')},
        radius: 300,
        inputBinding: {
            locationNameInput: $('.splms-gmap-address')
        },
        enableAutocomplete: true,
        onchanged: function(currentLocation, radius, isMarkerDropped) {
            $('.gmap-latlng').val( currentLocation.latitude + ", " + currentLocation.longitude );
        }
    });
});
